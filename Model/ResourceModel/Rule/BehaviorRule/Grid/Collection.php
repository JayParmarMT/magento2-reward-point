...
namespace Meetanshi\RewardPoints\Model\ResourceModel\Rule\BehaviorRule\Grid;
...
/**
 * Behavior Rule Grid Collection
 *
 * Extends the base behavior rule collection and joins websites and customer groups
 * for the admin grid display.
 */
class Collection extends BehaviorRuleCollection implements SearchResultInterface
{
    /**
    ...
    public function addFieldToFilter($field, $condition = null)
    {
        if ($field === 'websites') {
            ...
            $websiteSelect = $this->getConnection()->select()
                ...
                ->where('fw.rule_type = ?', 'behavior_earning')
        ...
        if ($field === 'customer_groups') {
            ...
            $groupSelect = $this->getConnection()->select()
                ...
                ->where('fg.rule_type = ?', 'behavior_earning')
