...
namespace Meetanshi\RewardPoints\Model\ResourceModel\Rule\CartRule\Grid;
...
/**
 * Cart Rule Grid Collection
 *
 * Extends the base cart rule collection and joins websites and customer groups
 * for the admin grid display.
 */
class Collection extends CartRuleCollection implements SearchResultInterface
{
    /**
    ...
    public function addFieldToFilter($field, $condition = null)
    {
        if ($field === 'websites') {
            ...
            $websiteSelect = $this->getConnection()->select()
                ...
                ->where('fw.rule_type = ?', 'cart_earning')
        ...
        if ($field === 'customer_groups') {
            ...
            $groupSelect = $this->getConnection()->select()
                ...
                ->where('fg.rule_type = ?', 'cart_earning')
