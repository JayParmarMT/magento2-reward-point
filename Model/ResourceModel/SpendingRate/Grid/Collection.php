...
namespace Meetanshi\RewardPoints\Model\ResourceModel\SpendingRate\Grid;
...
use Meetanshi\RewardPoints\Model\ResourceModel\SpendingRate\Collection as SpendingRateCollection;
use Psr\Log\LoggerInterface;
use Magento\Framework\DB\Expr as Zend_Db_Expr;

/**
 * Spending Rate Grid Collection
 *
 * Extends the base spending rate collection and joins websites and customer groups
 * via GROUP_CONCAT subqueries so both columns are filterable in the admin grid.
 */
class Collection extends SpendingRateCollection implements SearchResultInterface
{
    /**
    ...
    public function addFieldToFilter($field, $condition = null)
    {
        if ($field === 'websites') {
            ...
            $websiteSelect = $this->getConnection()->select()
                ...
                ->where('fw.rule_type = ?', 'spending_rate')
        ...
        if ($field === 'customer_groups') {
            ...
            $groupSelect = $this->getConnection()->select()
                ...
                ->where('fg.rule_type = ?', 'spending_rate')
