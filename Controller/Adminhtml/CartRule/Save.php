...
class Save extends Action implements HttpPostActionInterface
{
    ...
    public function __construct(
        ...
        private readonly CartRuleConditionFactory $conditionFactory,
    ...
    public function execute(): \Magento\Framework\Controller\Result\Redirect
    {
        ...
        try {
            ...
            $rule->setIsShownOnProductPage((bool) ($data['is_shown_on_product_page'] ?? 0));
            ...
            if (!empty($data['rule']['actions']) && is_array($data['rule']['actions'])) {
                ...
                $rule->setActionsSerialized($data['actions_serialized']);
            }
            ...
            $this->saveRuleLabels($savedRuleId, $data['store_labels'] ?? []);

            $this->messageManager->addSuccessMessage(__('The cart earning rule has been saved.'));
    ...
    /**
     * Save per-store-view rule labels
     *
     * @param int $ruleId
     * @param array $labels
     * @return void
     */
    private function saveRuleLabels(int $ruleId, array $labels): void
    {
        $connection = $this->resourceConnection->getConnection();
        $table = $this->resourceConnection->getTableName('meetanshi_rewardpoints_rule_label');
