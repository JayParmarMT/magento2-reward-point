...
class AddBuyWithPoints implements HttpGetActionInterface
{
    ...
    public function execute(): ResultInterface
    {
        ...
        try {
            ...
            if ($pointPrice <= 0) {
                $this->messageManager->addErrorMessage(__('This product is not available for purchase with reward points.'));
            ...
            if ($balance < $pointPrice) {
                $this->messageManager->addErrorMessage(
                    __('You need %1 points to buy this product. Your current balance is %2.',
            ...
            // Mark item as buy-with-points
            $quoteItem->setData('meetanshi_rp_buy_with_points', 1);
            $this->checkoutCart->save();

            $this->messageManager->addSuccessMessage(
                __('"%1" has been added to your cart. It will be purchased using %2 points at checkout.',
                    $product->getName(),
                    $this->config->formatPoints($pointPrice)
                )
            );
