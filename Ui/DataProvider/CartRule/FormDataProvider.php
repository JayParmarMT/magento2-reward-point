...
namespace Meetanshi\RewardPoints\Ui\DataProvider\CartRule;

use Magento\Framework\App\RequestInterface;
use Magento\Framework\App\ResourceConnection;
use Magento\Ui\DataProvider\AbstractDataProvider;
use Meetanshi\RewardPoints\Model\ResourceModel\Rule\CartRule\CollectionFactory;

/**
 * Form data provider for cart rule edit form
 */
class FormDataProvider extends AbstractDataProvider
{
    ...
    public function getData(): array
    {
        ...
        $ruleData['store_labels'] = $this->getRuleLabels($ruleId);
    ...
    /**
     * Get store labels for rule
     *
     * @param int $ruleId
     * @return array
     */
    private function getRuleLabels(int $ruleId): array
    {
        $connection = $this->resourceConnection->getConnection();
        $table = $this->resourceConnection->getTableName('meetanshi_rewardpoints_rule_label');
        $select = $connection->select()
            ->from($table, ['store_id', 'label'])
            ->where('rule_id = ?', $ruleId)
            ->where('rule_type = ?', 'cart_earning');
