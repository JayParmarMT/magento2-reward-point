...
/**
 * Points Earned Report block with chart data
 */
class Earned extends Template
{
    /**
    ...
    /**
     * Get earned points chart data grouped by period
     *
    ...
    public function getChartData(string $period = 'day'): array
    {
        ...
        $select = $connection->select()
            ->from(
                ...
                [
                    'date' => $dateExpr,
                    'points' => new \Magento\Framework\DB\Expr('SUM(points_delta)'),
                ],
            )
    ...
    public function getTotalEarned(): int
    {
        ...
        $select = $connection->select()
            ->from($tableName, [new \Magento\Framework\DB\Expr('SUM(points_delta)')])
            ->where('points_delta > 0')
            ->where('status IN (?)', ['active', 'pending']);
