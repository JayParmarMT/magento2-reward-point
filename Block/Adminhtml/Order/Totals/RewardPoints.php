...
namespace Meetanshi\RewardPoints\Block\Adminhtml\Order\Totals;
...
/**
 * Adds the Reward Points discount row to the admin order/invoice/creditmemo totals block.
 *
 * Works across four layout handles:
 *   sales_order_view         — source IS the order
 *   sales_order_invoice_view — source is invoice  → walk to getOrder()
 *   sales_order_invoice_new  — source is new invoice (unsaved) → walk to getOrder()
 *   sales_order_creditmemo_view / _new — source is creditmemo → walk to getOrder()
 *
 * Falls back to the core registry (current_order / current_invoice /
 * current_creditmemo) when the block graph hasn't been wired yet.
 */
class RewardPoints extends Template
{
    ...
    private function resolveOrder(): ?\Magento\Sales\Model\Order
    {
        ...
        if ($parent && method_exists($parent, 'getSource')) {
            ...
            if ($source) {
                // Already an order (sales_order_view)
                if ($source instanceof \Magento\Sales\Model\Order) {
                    return $source;
                }

                // Invoice or creditmemo — walk up to order
        ...
        // Fallback: registry (covers invoice/creditmemo new pages where
        // the invoice object may not yet have getOrder() populated)
